...
def build_match_sheet(events, lineup, match_id, matches_file=None):
    ...
    home_name, away_name = lineup[0]["team_name"], lineup[1]["team_name"]
    ...
    for idx, tid in enumerate((home_id, away_id)):
        ...
        tname = home_name if side == "home" else away_name
        sheet["teams"][side] = {"id": str(tid), "name": tname, "players": []}
    ...
    if matches_file:
        ...
        if match_rec:
            ...
            if comp_name is None:
                comp_name = (match_rec.get("competition", {}) or {}).get("competition_name") \
                          or match_rec.get("competition_name")
